...
# Android / 精简打包环境通常不含 PortAudio：避免顶层 import 导致应用一启动就崩
try:
...
def wasapi_shared_extra_settings(device: int | None):
    ...
    if sd is None or platform.system() != "Windows" or not hasattr(sd, "WasapiSettings"):
        return None
...
class PcmMicReader:
    ...
    def start(self) -> None:
        if sd is None:
            raise RuntimeError("当前环境未安装 sounddevice（PortAudio），无法使用 AUDIO_BACKEND=portaudio。")
